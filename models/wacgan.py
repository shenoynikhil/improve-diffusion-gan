...
from .gan_utils import compute_metrics


class Generator(nn.Module):
    """Generator Framework for WACGAN-GP"""
    ...
    def __init__(self, opt):
        ...
        self.main_module = nn.Sequential(
            # Z latent vector 100
            nn.ConvTranspose2d(
                in_channels=opt.latent_dim + opt.n_classes,
                out_channels=1024,
                kernel_size=4,
                stride=1,
                padding=0,
    ...
    def forward(self, z, label):
        label = F.one_hot(label, self.opt.n_classes).unsqueeze(2).unsqueeze(3)
...
class Discriminator(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.ac = nn.Sequential(nn.Linear(128, opt.n_classes), nn.Softmax())
...
class WACGAN_GP(ACGAN):
    """WACGAN_GP Network: An Extension of ACGAN"""
    ...
    def __init__(self, opt):
        ...
        self.critic_iter = opt.get("critic_iter", 5)
        # used to multiply this with gradient clipping value
        self.lambda_term = opt.get("lambda_term", 10)
    ...
    def _generate_labels(self, size):
        return torch.randint(0, self.opt.n_classes, (size,))
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        gen_labels = self._generate_labels(batch_size).type_as(labels)
        ...
        if optimizer_idx == 0:
            ...
            gen_pred, pred_label = self.discriminator(gen_imgs)

            # gen_pred_loss by mean across batch dim, shape => (batch_size, 1)
            gen_pred_loss = torch.mean(gen_pred)

            # auxillary loss
            gen_aux_loss = self.auxiliary_loss(pred_label, gen_labels)

            # compute g_loss
            g_loss = (gen_aux_loss - gen_pred_loss) / 2
        ...
        if optimizer_idx == 1:
            ...
            # 3. auxillary loss
            # 4. loss = (aux_loss +/- pred_loss) / 2 (+ for gem case, - for real)

            # For real imgs
            real_pred, real_aux = self.discriminator(imgs)
            real_pred_loss = torch.mean(real_pred)
            real_aux_loss = self.auxiliary_loss(real_aux, labels)
            real_loss = (real_aux_loss - real_pred_loss) / 2

            # For gen imgs
            fake_pred, fake_aux = self.discriminator(gen_imgs)
            fake_pred_loss = torch.mean(fake_pred)
            fake_aux_loss = self.auxiliary_loss(fake_aux, gen_labels)
            fake_loss = (fake_aux_loss + fake_pred_loss) / 2
