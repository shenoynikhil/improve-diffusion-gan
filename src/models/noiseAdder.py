...
class GNadder(BaseAdder):
    ...
    def apply_noise(self, t_img: torch.Tensor, gan=None):
        ...
        self.generated_noise = (
            self.noise_w * self.noise_cache[sample_idxs].to(t_img.device)
            + (1 - self.noise_w) * t_img
        )
    ...
    def log_noise(self, pl_module, n_epoch_render=10):
        ...
        if pl_module.current_epoch % n_epoch_render == 0:
            ...
            sample_image(
                gen_imgs=noise, n_row=10, epochs_done=pl_module.current_epoch, output_dir=save_path
            )
